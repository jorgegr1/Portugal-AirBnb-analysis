...
def workload_train_rf(spark: SparkSession, data_root: str) -> None:
    ...
    df = (
        ...
        .filter((F.col("price") >= 10) & (F.col("price") <= 1000))
        .fillna(0)
    )
...
def workload_review_demand(spark: SparkSession, data_root: str) -> None:
    ...
    result = (
        joined.groupBy("city", "year", "month", "price_range", "room_type")
        .agg(
...
def workload_calendar_revenue(spark: SparkSession, data_root: str) -> None:
    ...
    joined = joined.withColumn(
        "daily_revenue", F.when(~F.col("available"), F.col("price")).otherwise(F.lit(0.0))
    )

    # Aggregate by city, neighbourhood, year, month
    monthly = joined.groupBy("city", "neighbourhood_cleansed", "year", "month").agg(
        F.sum("daily_revenue").alias("total_revenue"),
        F.avg("daily_revenue").alias("avg_daily_revenue"),
        F.count("*").alias("n_days"),
        F.sum((~F.col("available")).cast("int")).alias("booked_days"),
        F.sum(F.col("available").cast("int")).alias("available_days"),
        F.countDistinct("listing_id").alias("n_listings"),
    )

    # Add occupancy rate
    monthly = monthly.withColumn(
        "occupancy_rate", F.col("booked_days") / (F.col("booked_days") + F.col("available_days"))
    )
...
def workload_full_pipeline(spark: SparkSession, data_root: str) -> None:
    ...
    review_counts = reviews.groupBy(
        "listing_id", F.year("date").alias("year"), F.month("date").alias("month")
    ...
    combined = occupancy.join(listings_slim, on="listing_id", how="inner").join(
        review_counts, on=["listing_id", "year", "month"], how="left"
    )
    combined = combined.fillna({"n_reviews_month": 0})

    # Step 4: aggregate by city, room_type, month
    result = (
        combined.groupBy("city", "room_type", "year", "month")
        .agg(
...
def main() -> None:
    ...
    p.add_argument(
        "--data-root", default=os.environ.get("DATA_ROOT", "gs://egd-airbnb-bucket/data")
    )
    args = p.parse_args()
