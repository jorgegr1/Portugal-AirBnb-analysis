...
from egd_airbnb.ml.features import engineer_features, select_and_fill
...
def _extract_importance(model: PipelineModel, sample_df) -> pd.Series:
    ...
    return pd.DataFrame(rows).groupby("feature")["importance"].sum().sort_values(ascending=False)
