...
with col1:
    ...
    municipality  = st.selectbox("Municipality",
                        sorted(listings[listings["city"] == city]["neighbourhood_group"].dropna().unique()), key="p_mun")
    neighbourhood = st.selectbox("Neighbourhood",
                        sorted(listings[listings["neighbourhood_group"] == municipality]["neighbourhood"].dropna().unique()), key="p_nb")

with col2:
    st.subheader("Property")
    room_type     = st.selectbox("Room type", sorted(listings["room_type"].dropna().unique()), key="p_rt")
    property_type = st.selectbox("Property type",
                        sorted(listings["property_type"].dropna().unique()) if "property_type" in listings else ["Entire rental unit"], key="p_pt")
...
with col3:
    ...
    host_listings  = st.slider("Host total listings", 1, 50, 1, key="p_hl",
...
if st.button("Predict nightly price", type="primary"):
    ...
    row_dict = engineer_row(
        ...
        host_is_superhost="false", instant_bookable="false",   # zero importance — use neutral
...
st.caption("Occupancy rate by month shows when your listing will face highest competition / demand.")
