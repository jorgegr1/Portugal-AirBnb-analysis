...
monthly_dir = next(
    iter(
        sorted(MODELS_DIR.glob("monthly_occupancy_*"), reverse=True) if MODELS_DIR.exists() else []
    ),
    None,
)
...
with col2:
    ...
    room_type = st.selectbox(
        "Room type", sorted(listings["room_type"].dropna().unique()), key="o_rt"
    )
...
with col3:
    ...
    target_month = st.selectbox(
        "Target month (for monthly model)",
...
if st.button("Predict occupancy", type="primary"):
    ...
    with tab_annual:
        if annual_dir:
            ...
            result = (
                model.transform(spark.createDataFrame([row_dict]))
            ...
            if pred == 1:
                ...
                st.write(
                    "Long-run occupancy predicted **>70%** — fewer than ~110 available days/year."
                )
            else:
    ...
    with tab_monthly:
        if monthly_dir and not str(monthly_dir).endswith(".json"):
            ...
            result_m = (
                model_m.transform(spark.createDataFrame([row_m]))
                .select("prediction", "probability")
                .first()
            )
            ...
            if pred_m == 1:
                ...
                st.info(
                    f"**Lower demand in {target_month}** ({(1 - prob_m) * 100:.1f}% confidence)"
                )
            st.progress(
                prob_m, text=f"High-demand probability for {target_month}: {prob_m * 100:.1f}%"
            )
        else:
