...
def regression_metrics(pred: DataFrame, *, label_col: str = "label", prediction_col: str = "prediction") -> dict:
    out = {}
    for m in ("rmse", "mae", "r2"):
        out[m] = float(
            RegressionEvaluator(labelCol=label_col, predictionCol=prediction_col, metricName=m).evaluate(pred)
        )
    return out


def regression_metrics_eur(pred: DataFrame, *, label_col: str = "price", prediction_col: str = "prediction_price") -> dict:
...
def classification_metrics(pred: DataFrame, *, label_col: str = "label", prediction_col: str = "prediction") -> dict:
