...
def _build_monthly_dataset(spark: SparkSession) -> DataFrame:
    ...
    calendar = read_parquet(spark, PROCESSED_DIR / "calendar").filter(F.col("date").isNotNull())
    ...
    monthly = (
        ...
        .agg(
            ...
            F.avg((~F.col("available")).cast("double")).alias("monthly_occupancy_rate"),
    ...
    joined = (
        joined.withColumn("month", F.col("month").cast("double"))
        .withColumn("sin_month", F.sin(F.col("month") * F.lit(2 * math.pi / 12)))
        .withColumn("cos_month", F.cos(F.col("month") * F.lit(2 * math.pi / 12)))
        .withColumn("is_high_demand_month", (F.col("monthly_occupancy_rate") > 0.50).cast("double"))
    )
    return joined
...
def train(spark: SparkSession, sample: float | None = None) -> dict:
    ...
    train_df = select_and_fill(train_fe, leak_cols=LEAK, extra_numeric=extra_num).withColumn(
        "label", F.col("is_high_demand_month")
    )
    test_df = select_and_fill(test_fe, leak_cols=LEAK, extra_numeric=extra_num).withColumn(
        "label", F.col("is_high_demand_month")
    )
    ...
    clf_kwargs: dict = dict(
        featuresCol="features", labelCol="label", numTrees=150, maxDepth=12, seed=42
    )
    if weight_col:
