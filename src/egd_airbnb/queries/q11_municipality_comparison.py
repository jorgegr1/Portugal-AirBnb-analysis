...
def run(spark: SparkSession) -> DataFrame:
    ...
    municipality = listings.groupBy("city", "neighbourhood_group_cleansed").agg(
    ...
    return (
        municipality.join(nb, ["city", "neighbourhood_group_cleansed"], "left")
        .withColumn(
            "listings_per_neighbourhood",
            F.col("n_listings") / F.col("n_neighbourhoods_in_municipality"),
        )
