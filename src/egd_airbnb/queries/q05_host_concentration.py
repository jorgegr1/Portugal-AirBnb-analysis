...
def run(spark: SparkSession) -> DataFrame:
    ...
    per_host = listings.groupBy("city", "host_id").agg(F.count("id").alias("n_listings"))
    ...
    ranked = (
        per_host.withColumn("rank", F.row_number().over(w))
        .withColumn(
            "cum_listings", F.sum("n_listings").over(w.rowsBetween(Window.unboundedPreceding, 0))
        )
        .join(totals, "city")
    ...
    for d in deciles:
        rows.append(
            ranked.filter(F.col("host_pct") <= d)
            .groupBy("city")
